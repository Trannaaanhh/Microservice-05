...
def clothes_list(request):
        ...
        if not is_staff:
            ...
            try:
                if action == "update_clothes":
                    ...
                    update_resp = requests.put(f"{CLOTHES_SERVICE_URL}/clothes/{clothes_id}/", json=payload, timeout=5)
                    if update_resp.status_code not in [200, 201]:
                    ...
                    clothes_id = _to_int(request.POST.get("clothes_id", "0"), 0)
                    delete_resp = requests.delete(f"{CLOTHES_SERVICE_URL}/clothes/{clothes_id}/", timeout=5)
                    if delete_resp.status_code not in [200, 204]:
                    ...
                    create_resp = requests.post(f"{CLOTHES_SERVICE_URL}/clothes/", json=payload, timeout=5)
                    if create_resp.status_code not in [200, 201]:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            error = f"Load clothes failed (status {response.status_code})"
    except requests.RequestException as exc:
        if not error:
            error = str(exc)
...
def clothes_detail(request, clothes_id):
    ...
    try:
        response = requests.get(f"{CLOTHES_SERVICE_URL}/clothes/{clothes_id}/", timeout=5)
        if response.status_code == 200:
            clothes = response.json()
        else:
            error = f"clothes not found (status {response.status_code})"
    except requests.RequestException as exc:
        error = str(exc)
